...
if (wishesForm) {
  wishesForm.addEventListener('submit', function (e) {
    ...
    .then(() => {
      // Simpan ucapan ke LocalStorage terlebih dahulu
      saveWishToLocal(nama, ucapan, kehadiran);

      // 🌟 PERBAIKAN: Beri jeda 2.5 detik agar Google Sheets selesai memproses baris baru
      setTimeout(() => {
      ...
      if (rsvpModal && closeRsvpModal) {
        ...
        closeRsvpModal.onclick = function () { rsvpModal.classList.remove('show'); };
        rsvpModal.onclick = function (event) { if (event.target === rsvpModal) rsvpModal.classList.remove('show'); };
      }
    })
    .catch((error) => {
